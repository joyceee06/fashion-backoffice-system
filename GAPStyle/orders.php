<?php
include 'auth.php';
redirectIfNoAccess('orders', 'read');
...
<head>
  ...
  <title>GAP STYLE Ordering System: Orders</title>
  <!-- Bootstrap -->
    ...
    <div class="row">
      <div class="col-xs-12 col-sm-8 col-sm-offset-2 col-md-6 col-md-offset-3">
        <div class="page-header">
          <h2>
            <?php echo isset($_GET['edit']) ? 'Edit Order' : 'Create New Order'; ?>
            </h2>
        </div>
          ...
          <div class="col-sm-9">
            <select name="cid" class="form-control" id="customer" required>
              ...
              try {
                $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
                $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
                $stmt = $conn->prepare("SELECT * FROM tbl_customers_a201392_pt2");
                $stmt->execute();
                $result = $stmt->fetchAll();
              }
              catch(PDOException $e){
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      ...
      <table class="table table-striped table-bordered">  
        ...
        try {
          $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
          $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
          $sql = "SELECT * FROM tbl_orders_a201392_pt2
          JOIN tbl_staffs_a201392_pt2
          ON tbl_orders_a201392_pt2.fld_staff_id = tbl_staffs_a201392_pt2.fld_staff_id
          JOIN tbl_customers_a201392_pt2
          ON tbl_orders_a201392_pt2.fld_customer_id = tbl_customers_a201392_pt2.fld_customer_id";
        ...
        foreach($result as $orderrow) {
          ...
          <tr>
            ...
            <td>
              <a href="orders_details.php?oid=<?php echo $orderrow['fld_order_num']; ?>" class="btn btn-warning btn-xs" role="button"> <span class="glyphicon glyphicon-eye-open"></span> Details</a>

              <?php if (canAccess('orders', 'update')): ?>
              <a href="orders.php?edit=<?php echo $orderrow['fld_order_num']; ?>" class="btn btn-success btn-xs" role="button">
                <span class="glyphicon glyphicon-edit"></span>
                Edit
              </a>
              <?php endif; ?>
              <?php if (canAccess('orders', 'delete')): ?>
              <a href="orders.php?delete=<?php echo $orderrow['fld_order_num']; ?>" onclick="return confirm('Are you sure to delete?');" class="btn btn-danger btn-xs" role="button">
    ...
    <div class="col-xs-12 col-sm-10 col-sm-offset-1 col-md-8 col-md-offset-2">
      <nav>
          ...
          try {
            $conn = new PDO("mysql:host=$servername;dbname=$dbname", $username, $password);
            $conn->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_a201392_pt2");
            $stmt->execute();
            $result = $stmt->fetchAll();
