          ...
          <div class="col-sm-9">
            <select name="pid" class="form-control" id="prd">
              ...
              foreach($result as $productrow) {
                ?>
                <option value="<?php echo $productrow['fld_product_id']; ?>"><?php echo $productrow['fld_category']." ".$productrow['fld_product_name']; ?></option>
              <?php }
              $conn = null;
              ?>
            </select>
        ...
        <tbody>
          ...
          try {
            ...
            $stmt = $conn->prepare("SELECT * FROM tbl_orders_details_a201392_pt2, tbl_products_a201392_pt2 
              WHERE tbl_orders_details_a201392_pt2.fld_product_id = tbl_products_a201392_pt2.fld_product_id AND fld_order_num = :oid");
            $stmt->bindParam(':oid', $oid, PDO::PARAM_STR);
            $oid = $_GET['oid'];
            $stmt->execute();
            $result = $stmt->fetchAll();
